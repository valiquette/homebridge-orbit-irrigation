...
class sensor {
	...
	async getStatusLowBattery(batteryStatus, callback) {
		...
		try{
			let sensorResponse = (await this.orbitapi.getDevice(this.platform.token, batteryStatus.subtype).catch(err=>{this.log.error('Failed to get device response %s', err)}))
		...
		}catch(err){this.log.error('Failed to read sensor', err)}
