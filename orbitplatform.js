...
//review detail
  updateService(message) {
    try{
      ...
      let uuid = UUIDGen.generate(jsonData.device_id)
      let irrigationAccessory = this.accessories[uuid]
      let bridgeAccessory = this.accessories[uuid]
      let irrigationSystemService = irrigationAccessory.getService(Service.IrrigationSystem)
      let bridgeService = bridgeAccessory.getService(Service.BridgeConfiguration)
      let switchService = irrigationAccessory.getServiceById(Service.Switch,UUIDGen.generate(jsonData.device_id+' Standby'))
      let activeService
      let deviceName=this.deviceGraph.devices.filter( result => result.id == jsonData.device_id)[0].name
      ...
      False	  True	  Stopping
      ******************************/
      if (this.showIncomingMessages){this.log.warn('incoming message',jsonData)} //additional debug info
      if (this.lastMessage==message){return} //suppress duplicate websocket messages
      this.lastMessage=message
      
      switch (jsonData.event) {
        case "watering_in_progress_notification":
          if(jsonData.source!='local'){this.log.info('Watering in progress device %s for %s mins',deviceName, Math.round(jsonData.run_time))}
          activeService = irrigationAccessory.getServiceById(Service.Valve, jsonData.device_id)
          if (activeService){
            ...
            activeService.getCharacteristic(Characteristic.RemainingDuration).updateValue(jsonData.run_time * 60)
            let endTime= new Date(Date.now() + parseInt(jsonData.run_time) * 60 * 1000).toISOString()
            activeService.getCharacteristic(Characteristic.CurrentTime).updateValue(endTime)
          }
        break
        case "watering_complete":
          if(jsonData.source!='local'){this.log.info('Device %s watering completed',deviceName)}
        ...
        break
        case "device_idle":
          if(!irrigationSystemService){break} 
        ...
        break
        case "change_mode":
          this.log.debug('%s mode changed to %s',deviceName,jsonData.mode)
          if(!irrigationSystemService){break}
        ...
        case "device_connected":
          this.log.info('%s device connected',deviceName)
          if(bridgeService){bridgeService.getCharacteristic(Characteristic.StatusFault).updateValue(Characteristic.StatusFault.NO_FAULT)}
          if(irrigationSystemService){irrigationSystemService.getCharacteristic(Characteristic.StatusFault).updateValue(Characteristic.StatusFault.NO_FAULT)}
        break
        case "device_disconnected":
          this.log.info('%s device disconnected',deviceName)
          if(bridgeService){bridgeService.getCharacteristic(Characteristic.StatusFault).updateValue(Characteristic.StatusFault.GENERAL_FAULT)}
          if(irrigationSystemService){irrigationSystemService.getCharacteristic(Characteristic.StatusFault).updateValue(Characteristic.StatusFault.GENERAL_FAULT)}
        break
        case "clear_low_battery":
          this.log.debug('%s low battery cleared',deviceName)
          activeService = irrigationAccessory.getServiceById(Service.Battery, jsonData.device_id)
          if (activeService){
        ...
        break
        default:
          this.log.warn('Unknown message received: %s',jsonData.event);
        break
      }
      return
